          ...
          {academicProjects.map((p, i) => (
            <Reveal key={p.id} delay={i * 90}>
              <article className="group glass relative flex h-full flex-col overflow-hidden rounded-2xl p-6 transition-all hover:-translate-y-1 hover:glow-ring">
                {/* image placeholder */}
                <div className="mb-5 flex aspect-video items-center justify-center overflow-hidden rounded-xl border border-border bg-background/40">
                  {p.image ? (
                ...
                <div className="mb-2 flex items-center justify-between">
                  <span className="font-mono text-xs text-cyan">{p.index}</span>
                  <ArrowUpRight
                    size={16}
